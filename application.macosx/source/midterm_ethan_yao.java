...
public class midterm_ethan_yao extends PApplet {
...
float bossKnightDifficulty = 0.0f;
...
public void loadAssets() // Loads Game Assets 
{
  // Font Assets
  ...
  knightSlashSound.amp(0.5f);
  knightDeathSound.amp(0.5f);
  knightDamageSound.amp(0.4f);
  knightDefendSound.amp(0.25f);
  knightYellSound.amp(0.8f);
  
  smallEnemyDeathSound.amp(0.3f);
  smallEnemySlashSound.amp(0.1f);
}

public void createEnemyKnights() // Function To Create Small Enemy Knights (Reduce Code In Setup And To Easily Reset Enemies)
{
  ...
  {                          // And The Range At Which Enemies Decide When To Attack
    knightBossFrameRate = PApplet.parseInt(random(8, 12));
    knightBossAttackFrameRate = PApplet.parseInt(random(12, 18));
    bossKnightDifficulty = PApplet.parseInt(random(0.4f, 0.5f));
  }
  else if (gameDifficulty == "normal")
  {
    knightBossFrameRate = PApplet.parseInt(random(10, 14));
    knightBossAttackFrameRate = PApplet.parseInt(random(16, 22));
    bossKnightDifficulty = PApplet.parseInt(random(0.3f, 0.4f));
  }
  else if (gameDifficulty == "easy")
  {
    knightBossFrameRate = PApplet.parseInt(random(12, 16));
    knightBossAttackFrameRate = PApplet.parseInt(random(20, 26));
    bossKnightDifficulty = PApplet.parseInt(random(0.2f, 0.3f));
  }
...
public void reset(boolean trueRestart) // Resets All Vars
{
...
{ 
  if (gameState == "menu")
  {
    ...
    textSize(24);
    text("Normal", 13 * width/32, height/2 + (0.5f * textHeightDiff), 15 * width/16, height);
  ...
  if (gameState == "title") // Title State With Triggers For Text
  {
    ...
    if (textState >= 0 && textState < 2) 
    {
      ...
      text("Midterm Project", textWidth * 0.5f, textHeight * 2.25f);
      
      if (textState == 0) image(downArrowContinue[utilFrame], textWidth * 4.75f, textHeight * 2.25f);
    }
    if (textState == 1)
    {
      text("by Ethan Yao", textWidth * 3, (textHeight * 2.25f) + textHeightDiff);
      
      image(downArrowContinue[utilFrame], textWidth * 6.5f, (textHeight * 2.25f) + textHeightDiff);
    }
    if (textState >= 2) 
    {
      ...
      text(descrip, textWidth * 0.5f, textHeight * 0.5f, width - textWidth, textHeight + textHeightDiff);
      
      if (textState == 2) image(downArrowContinue[utilFrame], width - (textWidth * 0.4f), textHeight * 1.35f);
    }
    if (textState >= 3) 
    {
      ...
      text(descrip, textWidth * 0.5f, textHeight * 1.75f, width - textWidth, textHeight + (1.25f * textHeightDiff));
      
      if (textState == 3) image(downArrowContinue[utilFrame], width - (textWidth * 0.4f), textHeight * 2.5f);
    }
    if (textState >= 4)
    {
      text("Game Objective: ", textWidth * 0.5f, textHeight + (1.5f *textHeightDiff));
      
      if (textState == 4) image(downArrowContinue[utilFrame], textWidth * 2.9f, textHeight + (1.48f *textHeightDiff));
    }
    if (textState >= 5)
    {
      text("-Explore The Caverns", textWidth * 1.5f, textHeight + (2 * textHeightDiff));
      
      if (textState == 5) image(downArrowContinue[utilFrame], textWidth * 4.8f, textHeight + (1.975f *textHeightDiff));
    }
    if (textState >= 6)
    {
      text("-Kill The Monster Hidden Within", textWidth * 1.5f, textHeight + (2.5f * textHeightDiff));
      
      if (textState == 6) image(downArrowContinue[utilFrame], textWidth * 6.85f, textHeight + (2.47f *textHeightDiff));
    }
  }
  else if (gameState == "help") // Simple Help Screen (Can Be Used As Pause State)
  {
    ...
    text("-Use the Left and Right arrow keys to move", textWidth * 0.25f, textHeight);
    text("-Press A to attack", textWidth * 0.25f, textHeight + (0.5f * textHeightDiff));
    text("-Press D to defend", textWidth * 0.25f, textHeight + (1 * textHeightDiff));
    text("-Press H to open up the help menu", textWidth * 0.25f, textHeight + (1.5f * textHeightDiff));
    text("-Press R to reset the game back to the title menu", textWidth * 0.25f, textHeight + (2 * textHeightDiff));
    text("-Press Enter to close the help menu", textWidth * 0.25f, textHeight + (2.5f * textHeightDiff));
    ...
    image(downArrowContinue[utilFrame], width - (1.3f * textWidth), textHeight + (2.45f * textHeightDiff));
  }
  ...
  { 
    ...
    fill(255);
    textFont(helpFont);
    text("Player Health", width/16, 1.5f * height/32);
    image(playerKnight.life[playerKnight.lifeFrame], 2.25f * width/16, height/16);
    ...
    else if (playerKnight.action == "attackRight") // Player Attack Animations
    {
      ...
      image(playerKnight.knightAttackRight[playerKnight.currFrame], playerKnight.knightPos.x + 
          (0.5f * playerKnight.knightAttackRight[0].width), playerKnight.knightPos.y);
      ...
      if (playerKnight.drawsTillAttackFrame <= 0)
      {
        ...
        {          
          if (bossKnight.knightPos.x - playerMoveOffset - width/2 - (0.5f * playerKnight.knightAttackRight[0].width) -
      ...
      {
        if (enemyList[i].enemyPos.x - playerMoveOffset - width/2 - (0.5f * playerKnight.knightAttackRight[0].width) - 
    ...
    else if (playerKnight.action == "attackLeft") 
    {
      ...
      image(playerKnight.knightAttackLeft[playerKnight.currFrame], playerKnight.knightPos.x - 
          (0.5f * playerKnight.knightAttackLeft[0].width), playerKnight.knightPos.y);
            ...
            if (enemyList[i].enemyPos.x - playerMoveOffset - width/2 - (enemyList[i].smallEnemyDifficulty * enemyAttack[0].width) - enemySlashLeft.width <= 0)
            {
              ...
              if (enemyList[i].currFrame == 1) // Checks for slash timing
              {
                image(enemyAttack[enemyList[i].currFrame], enemyList[i].enemyPos.x - playerMoveOffset - 
                    (0.5f * enemyAttack[0].width), enemyList[i].enemyPos.y);
                image(playerKnight.knightSlashLeft, enemyList[i].enemyPos.x - playerMoveOffset - 
                    (0.5f * enemyAttack[0].width), playerKnight.knightPos.y);
    ...
    if (bossKnight.alive && !gamePause)
    {
      if ((playerMoveOffset >= 1600 || bossKnight.isCreated) && !bossKnight.isDying)
      {
        ...
        fill(255);
        textFont(helpFont);
        text("Boss Health", 12 * width/16, 1.5f * height/32);
        ...
        if (lastScene && textState <= 1) // Creates Text Dialog Box
        {
          ...
          fill(0);
          rect(width/4, 8.5f * height/10, 2 * width/4, height/10);
          ...
          if (textState == 0) 
          {
            text("YOU MONSTER ! ", 1.6f * width/4, 9.1f * height/10);
            image(downArrowContinue[utilFrame], 1.22f * width/2, 9.0f * height/10);
          }
          
          else if (textState == 1) 
          {
            text("HOW COULD YOU ! ", 1.6f * width/4, 9.1f * height/10);
            image(downArrowContinue[utilFrame], 1.3f * width/2, 9.0f * height/10);
          }
        }
        // Create Cutscene #1
        ...
        {          
          ...
          if (bossKnight.currFrame == 1)  // Check to make sure slash art appears at right timing
          {
            image(bossKnight.knightAttackLeft[bossKnight.currFrame], bossKnight.knightPos.x - playerMoveOffset - 
                (0.5f * bossKnight.knightAttackLeft[0].width), bossKnight.knightPos.y);
...
class Knight
{
  ...
  public void reset(boolean trueRestart) // Reset Function
  {
    if (trueRestart)
    {
      lifeFrame = 3;
  ...
  public void die() // Knight Die Function (Mainly Used By playerKnight)
  {
    currFrame = 0;
  ...
  public void takeDamage() // Damage Function (Only Used By bossKnight)
  {
    lifeFrame--;
    knightPos.x += 4;
  ...
  public void defend() // Defend Function (Only Used By playerKnight)
  {
    knightDefendSound.play(1, 0, 0.4f);
...
class enemyKnight
{
  ...
  int enemyFrameRate = PApplet.parseInt(random(6, 10));
  int enemyAttackFrameRate = PApplet.parseInt(random(10, 14));
  int enemyOrigPosX;
  int currFrame = 0;
  int drawsTillNextFrame = enemyFrameRate;
  int drawsTillAttackFrame = enemyFrameRate;
  
  float smallEnemyDifficulty = random(0.2f, 0.5f);
  
  String action = "";

  public void setup(int xPos)
  { 
    String loadAssetName;
    ...
    {
      enemyFrameRate = PApplet.parseInt(random(4, 8));
      enemyAttackFrameRate = PApplet.parseInt(random(6, 10));
      smallEnemyDifficulty = random(0.4f, 0.8f);
    }
    else if (gameDifficulty == "normal")
    {
      enemyFrameRate = PApplet.parseInt(random(6, 10));
      enemyAttackFrameRate = PApplet.parseInt(random(8, 12));
      smallEnemyDifficulty = random(0.4f, 0.65f);
    }
    else if (gameDifficulty == "easy")
    {
      enemyFrameRate = PApplet.parseInt(random(8, 12));
      enemyAttackFrameRate = PApplet.parseInt(random(10, 14));
      smallEnemyDifficulty = random(0.4f, 0.5f);
    }
  ...
  public void reset() // Resets Enemy To Original
  {
    isCreated = false;
    isAlive = true;
    isDying = false;
...
*/
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "midterm_ethan_yao" };
    if (passedArgs != null) {
